...
def selectClass():
    cancelButton = '<button onclick="window.location.replace(\'/\');" style="position: relative;  margin-top: 10px;">Cancel</button>'
...
def doList():
    ...
    for i in table_rows:
        ...
        if i[5] in partner_id and i[5] != 0:
            current_partner = partner_fname[i[5] - 1] + " " + partner_lname[i[5] - 1]
        student_table += '<tr>' + '<td style="width:200px;"><input type="checkbox" name="add" value=' + str(i[0]) + '>' + str(i[1]) + ' ' + str(i[2]) + '</td><td>' + str(i[3]) + '</td><td>' + coding_level + '</td><td>' + str(current_partner) + '</td><td class="editContainer"><a href="/form?student_id=' + str(i[0]) + '&gender=' + str(i[3]) + '&coding_level=' + str(i[4]) + '&preferred_partner=' + str(i[5]) + '" class="editLink"><img src="../editIcon.png" title="Edit" alt="Edit"></a></td><tr>'                                                     
    student_table += "</table>"
    #print(student_table)
    #return "test"
...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("UPDATE students SET gender = '" + gender + "' WHERE student_id = '" + student_id + "'")
        mysql.connection.commit()
        cursor.execute("UPDATE students_classes SET coding_level = '" + coding_level + "', preferred_partner = '" + preferred_partner + "' WHERE student_id = '" + student_id + "' and class_id = '" + class_id + "'")
        mysql.connection.commit()
        cursor.close()
...
def generateClassTable(class_id):
    ...
    for i in current_students:
        student_table += '<tr> <td style="text-align: center; width: 30px;"><input checked type="checkbox" name="addStudent" value=' + str(i[0]) + '></td><td>' + str(i[1]) + ' ' + str(i[2]) + '</td><td class="editContainer"><a href="/form?student_id=' + str(i[0]) + '&gender=' + str(i[3]) + '&coding_level=' + str(i[4]) + '&preferred_partner=' + str(i[5]) + '&class_name=' + class_name[0] + '&class_id=' + class_id + '&student_entry=false" class="editLink"><img width="13" src="static/editIcon.png" title="Edit" alt="Edit"></a></td></tr>'                                                   
    ...
    for i in table_rows:
        if i[0] not in roster:
            student_table += '<tr><td style="text-align: center; width: 30px;"><input type="checkbox" name="addStudent" value=' + str(i[0]) + '></td><td>' + str(i[1]) + ' ' + str(i[2]) + '</td></tr>'  
...
def updateClass(): 
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT student_id from students_classes WHERE class_id='" + class_id + "'")
    ...
    for x in originalRosterList:
       if int(x) not in add_student:
        print("Delete:")
        print(x)
        cursor.execute("DELETE FROM students_classes WHERE class_id='" + str(class_id) + "' AND student_id='" + str(x) + "'")
        mysql.connection.commit()
    for y in insertList:
            print("Insert:")
            print(y)
            cursor.execute("INSERT INTO students_classes VALUES ('" + str(class_id) + "', '" + str(y) + "', 0, 0)")
            mysql.connection.commit()
    cursor.close()
    okayButton = '<button onclick="window.location.replace(\'/\');" style="position: relative;  margin-top: 10px;"">Back to dashboard</button>'
    return render_template('selectClass.html', student_table = generateClassTable(class_id), user_info = getUser(), class_options = getClasses(), save_button = "<button>Update Class</button>", okay_button = okayButton)    
...
def printResults():
    ...
    aString = ""
    for i in table_rows:
        aString += "<option value=" + str(i[0]) + ">" + str(i[1]) + " " + str(i[2]) + "</option>"
    return f"{aString}"

def getUser():
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT prefix, first_name, last_name FROM teachers WHERE teacher_id = " + str(user_id))
...
def getClasses():
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT class_id, class_name FROM teachers t JOIN classes c ON t.teacher_id = c.teacher_id WHERE t.teacher_id = " + str(user_id))
    ...
    for i in option_rows:
        if str(class_id) == str(i[0]):
            aString += "<option selected value=" + str(i[0]) + ">" + str(i[1]) + "</option>"
        else:
            aString += "<option value=" + str(i[0]) + ">" + str(i[1]) + "</option>"
    aString += '</select>'
...
def createGroups():
    ...
    hiddenFields += '<input type="hidden" name="assignmentTitle" value="' + assignment_title + '">'
    hiddenFields += '<input type="hidden" name="assignmentDescription" value="' + assignment_description + '">'
...
def showGroups():
    ...
    hiddenAssignmentFields = '<input type="hidden" name="assignmentTitle" value="' + assignmentTitle + '">'
    hiddenAssignmentFields += '<input type="hidden" name="assignmentDescription" value="' + assignmentDescription + '">'
    hiddenAssignmentFields += '<input type="hidden" name="teacher_id" value="' + str(user_id) + '">'
    hiddenAssignmentFields += '<input type="hidden" name="classId" value="' + str(classId) + '">'
    ...
    finalList = pairing_algorithm_v3.group(getStudentList(classId), isGenderImportant, isAbilityImportant, isPreferenceImportant)
    ...
    for x in finalList:
        ...
        #last_assignment
        for y in x:
            hiddenStudentFields += '<input type="hidden" name="gs" value="'+ str(groupDisplayNum) + '|' + str(y[1]) + '">'
...
def getStudentList(classId):
    ...
    cursor.execute("SELECT assignment_id FROM assignments ORDER BY assignment_id DESC LIMIT 1;")
    ...
    for i in range(len(student_list)):
        ...
        id = student_list[i][1]
        sqlStatement = "SELECT group_id from student_groups WHERE student_id='" + str(id) + "'  and assignment_id >= '" + str(int(last_assignment) - 2)  + "'"
        #print(sqlStatement)
...
def showAssignments():
    ...
    sqlStatement = "select assignment_name, DATE_FORMAT(assignment_date,'%m/%d/%Y'), assignment_id, assignment_description, class_id from assignments where teacher_id = '" + str(user_id) + "' ORDER BY assignment_id DESC"
    cursor.execute(sqlStatement)
...
def insertAssignment():
    ...
    assignmentInsert = "INSERT INTO assignments (assignment_id, assignment_date, assignment_name, assignment_description, teacher_id, class_id) VALUES ('" + str(assignmentNum) + "', CURDATE(), '" + str(assignment_name) + "','" + str(assignment_description) + "','" + str(user_id) + "','" + str(classId) + "')"
    ...
    for x in group_criteria:
        criteriaInsert = "INSERT INTO assignments_criteria (assignment_id, criteria_id) VALUES ('" + str(assignmentNum) + "', '" + str(x) + "')"
    ...
    for y in group_student:
        ...
        studentId = gs[1]
        studentInsert = "INSERT INTO student_groups (group_id, student_id, assignment_id) VALUES ('" + str(groupId) + "', '" + str(studentId) + "', '" + str(assignmentNum) + "')"
        print(studentInsert)
...
def assignmentSheet():
    ...
    class_id = request.form['classId']
    cursor = mysql.connection.cursor()
    groupNumsQuery = "SELECT distinct(group_id) FROM student_groups sg JOIN assignments a ON sg.assignment_id = a.assignment_id WHERE sg.assignment_id = '" + str(assignment_id) + "' and a.teacher_id = '" + str(user_id) + "' ORDER BY sg.assignment_id ASC"
    ...
    for x in groupNums:
        ...
        studentNamesQuery = "SELECT CONCAT_WS(' ',s.first_name,s.last_name) FROM student_groups sg JOIN students s ON sg.student_id = s.student_id JOIN assignments a ON a.assignment_id = sg.assignment_id WHERE sg.assignment_id = '" + str(assignment_id) + "' and a.teacher_id = '" + str(user_id) + "' and sg.group_id = '" + str(x[0]) + "' ORDER BY sg.group_id ASC"
        print(studentNamesQuery)
        cursor.execute(studentNamesQuery)
        studentNames = cursor.fetchall()
        tableGroupsDisplay += groupsHistoryRow(studentNames[0][0],studentNames[1][0],groupDisplayNum)
        groupDisplayNum = groupDisplayNum + 1
    tableGroupsDisplay += "</table>"
    tableGroupsDisplay += '<table class="criteriaKey">'
    tableGroupsDisplay += "<tr><th>Grouped based on:</th></tr>"
...
def getClassName(id):
    cursor = mysql.connection.cursor()
    sqlQuery = "SELECT class_name FROM classes WHERE class_id = '" + str(id) + "'"
    cursor.execute(sqlQuery)
